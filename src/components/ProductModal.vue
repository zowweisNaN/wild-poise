...
<template>
  <Teleport to="body">
    ...
    >
      ...
      >
        <div
          class="absolute inset-0 bg-slate-deep/60 backdrop-blur-sm"
          @click="emit('close')"
        />

        <div
          class="relative w-full sm:max-w-4xl max-h-[95vh] sm:max-h-[90vh] bg-soft-cream rounded-t-3xl sm:rounded-3xl shadow-modal overflow-hidden flex flex-col animate-modal-in"
        >
          <div
            class="flex items-center justify-between px-5 sm:px-8 pt-5 pb-4 border-b border-slate-grey/20 flex-shrink-0"
          >
          ...
          <div class="flex-1 overflow-y-auto">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-0">
              ...
              <div class="p-5 sm:p-8 flex flex-col gap-5">
                <div>
                  <h2
                    class="font-display font-bold text-slate-deep text-xl sm:text-2xl leading-snug mb-2"
                  >
                    {{ product.title }}
                  </h2>
