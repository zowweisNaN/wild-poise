...
import { ShieldCheck, KeyRound, Mail, Lock, Eye, EyeOff, X, LogIn, AlertCircle, Loader2 } from '@lucide/vue'
...
<template>
  <div class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
    ...
    >
      <div class="bg-white w-full max-w-md rounded-3xl shadow-2xl border border-slate-grey/20 overflow-hidden">
        <!-- Header -->
        <div class="bg-slate-deep px-7 pt-7 pb-6 relative">
          <button
            @click="emit('close')"
            class="absolute top-4 right-4 text-slate-grey/60 hover:text-white transition-colors p-1"
          >
            <X class="h-5 w-5" />
          </button>
          ...
          <div class="flex items-center gap-3 mb-2">
            ...
            <div>
              <h2 class="font-display font-bold text-xl text-white tracking-tight">Admin Access</h2>
        ...
        <div class="px-7 py-6 space-y-5">
          ...
          <div v-if="activeTab === 'passcode'" class="space-y-4">
            <p class="text-xs text-slate-grey leading-relaxed">
              ...
              <code class="bg-slate-100 px-1 py-0.5 rounded text-slate-deep font-mono text-[11px]">VITE_ADMIN_PASSCODE</code> environment variable.
            </p>
